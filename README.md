...
Приложение читает файлы csv и формирует отчёты. Названия файлов передаются в виде параметра __--files__ (имена файлов передаютс через пробел), тип отчёта - в виде __--report__ (существует только отчёт clickbait, если параметр не указать, будет выведен список всех данных из файлов в консоль). Для работы скрипта необходимо установить библиотеку tabulate (_pip install tabulate_).
#### Примеры запуска: 
...
## test_main.py
К заданию прилагаются тесты pytest (необходима установка библиотеки pytest):
